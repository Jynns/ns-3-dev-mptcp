...
class MpTcpSocketBase : public TcpSocketBase //: public TcpSocketBase
{
  public: // public methods
    ...
    virtual int Bind(
        const Address& address); // Bind a socket ... to specific add:port; set net device befor
    ...
    virtual void SetCongestionAgent(
        Ptr<MpTcpCongestionControlAgent>); //<! Object with parent MpTcpCongestionControlAgent TODO move this to
                             //protected and fix accessor
  ...
  protected:
    ...
    virtual Ptr<MpTcpCongestionControlAgent> GetCongestionAgent(); //<! returns an instance of
                                                                   //MpTcpCongestionControlAgent to
                                                                   //be used for the subflow
    virtual void SetCongestionInfoType(
        std::string typeID); //<! Object with parent MpTcpCongestionControlAgent
    virtual Ptr<CongestionInfo> GetCongestionInfo(); //<! returns an instance of
                                                     //MpTcpCongestionControlAgent to be used for
                                                     //the subflow
    ...
    // managing RL
    void UpdateCCInfo(uint8_t sFlowIdx);

    // helper function

    void IsLastAck();
    // Helper functions -> main operations
    // uint8_t LookupByAddrs(Ipv4Address src, Ipv4Address dst); // Called by Forwardup() to find the
    // right subflow for incoing packet
    virtual int LookupSubflow(Ipv4Address src,
                              uint32_t srcPort,
                              Ipv4Address dst,
                              uint32_t dstPort); // LookupBy4-Tuple

    virtual uint8_t getSubflowToUse(); // Called by SendPendingData() to get a subflow based on
                                       // round robin algorithm
    // bool IsThereRoute(Ipv4Address src, Ipv4Address dst);     // Called by InitiateSubflow &
    // LookupByAddrs and Connect to check whether there is route between a pair of addresses. bool
    // IsLocalAddress(Ipv4Address addr); bool IsRemoteAddress(Ipv4Address addr); Ptr<NetDevice>
    // FindOutputNetDevice(Ipv4Address);         // Find Netdevice object of specific IP address.
    ...
    Ptr<MpTcpCongestionControlAgent> m_ccAgent;
    // std::string m_ccAgentType; //<! Typeid of Congestion control agent that is called (old because lookupbyname didnt  work as intended)
    std::string m_ccInfoType;  //<! Typeid of Congestion control info that is used by the agent
    bool m_isCCAgent;
